...
# $HeadURL: http://svn.collab.net/repos/svn/branches/1.6.x/tools/backup/hot-backup.py.in $
# $LastChangedDate: 2009-01-25 06:09:04 +0000 (Sun, 25 Jan 2009) $
# $LastChangedBy: arfrever $
# $LastChangedRevision: 35454 $
...
p = subprocess.Popen([ "which", 'svnlook' ],
    stdin=subprocess.PIPE,
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE
)
outfile = p.stdout
svnlook = outfile.readlines()[0].strip()
#svnlook = "/usr/local/bin/svnlook"

# Path to svnadmin utility
#svnadmin = r"@SVN_BINDIR@/svnadmin"
p = subprocess.Popen([ "which", 'svnadmin' ],
    stdin=subprocess.PIPE,
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE
)
outfile = p.stdout
svnadmin = outfile.readlines()[0].strip()
...
try:
  ...
  sys.stderr.write("Main: Could not get youngest revision: `%s'\n" % e)
  sys.stderr.flush()
  sys.exit(1)
